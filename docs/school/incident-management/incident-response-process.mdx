...
Bro, a severity-1 alert just fired: the main checkout API is returning 500s. Three engineers jump onto a Slack call. Engineer A thinks it's a database lock and starts frantically killing queries. Engineer B thinks it's a bad deployment and starts reverting the last PR. Engineer C thinks it's a DDoS attack and starts blocking IPs. Within 10 minutes, they've completely corrupted the database and taken down the entire site, turning a minor issue into a catastrophic outage. This happens when there is no structured incident response process. A proper process forces discipline: first you *Triage* (acknowledge the alert), then you *Coordinate* (appoint an Incident Commander to stop everyone from stepping on each other), then you *Mitigate* (stop the bleeding, don't worry about the root cause yet), and finally you *Resolve*. Process beats panic every time.

## 3. Real-World Analogy

{/* TODO: [BAE-ILLUSTRATION] Generate and embed illustration here. Prompt: A grayscale line-art illustration of Bae, a software engineer, interacting with a emergency room (er) scenario: paramedic brings patient in */}

**FDE Scenario:** As an FDE dealing with complex client deployments, you don't have the luxury of time during an outage. When a production system goes down, your priority is to mitigate the impact immediately (e.g., by rolling back or isolating the service) rather than diagnosing the deep root cause while the system is still bleeding.
...
![Illustration of Bae as an ER doctor treating a bleeding patient first](./assets/incident-response-process.png)
...
|---|---|---|
| Mitigation First | Suggest mitigation strategies (e.g., disabling a feature flag or reverting a commit) before proposing complex bug fixes during an active incident. | FDE environments prioritize uptime over immediate root cause analysis. |
| Incident Scoping | Explicitly define the boundaries of the incident (what is affected and what is not) in initial reports. | Helps prevent scope creep and unnecessary panic across client teams. |
