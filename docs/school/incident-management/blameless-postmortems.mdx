...
Bro, imagine an intern accidentally drops the production database, causing a 4-hour outage. In a traditional, toxic company culture, the VP of Engineering immediately fires the intern to "make an example of them." What happens next? Every other engineer on the team becomes terrified. When they make a mistake, they hide it, secretly patching servers in the middle of the night to avoid being fired. The system becomes a fragile house of cards built on fear. In a strong engineering culture, you hold a Blameless Postmortem. You don't ask "Who dropped the database?" You ask "Why was it possible for a junior engineer's local script to access production credentials?" and "Why did our backup system fail to restore in under an hour?" Blameless postmortems fix the system, not the person.

## 3. Real-World Analogy

{/* TODO: [BAE-ILLUSTRATION] Generate and embed illustration here. Prompt: A grayscale line-art illustration of Bae, a software engineer, interacting with a post-match cricket analysis scenario: the match is over (we lost) */}

**FDE Scenario:** As an FDE, you will often debug outages caused by third-party legacy APIs on client infrastructure. When a client integration fails, your goal is to find out why the system allowed the failure without alerting you, not who deployed the bad code. Blameless culture helps you uncover structural weaknesses in unfamiliar client environments.
...
![Illustration of Bae handing sunglasses to a cricket player who dropped a catch](./assets/blameless-postmortems.png)
...
|---|---|---|
| Log Output | Output clear, actionable error logs when an operation fails. Do not write logs that assign blame (e.g., "User supplied bad data"). | FDE deployments involve client data that is often messy. Blame-oriented logs distract from finding the systemic fix. |
| Incident Documentation | When drafting postmortems, focus purely on timelines, commands run, and system states. | Objective documentation prevents defensive behavior and enables the team to build better guardrails. |
