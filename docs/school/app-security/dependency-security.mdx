...
{/* TODO: [BAE-ILLUSTRATION] Generate and embed illustration here. Prompt: A complex chain of colorful dominoes where one weak link threatens to topple the whole structure. */}
...
|---|---|---|
| Automated Scanning | Run SCA (Software Composition Analysis) tools on all builds and pull requests. | Catches known vulnerabilities (CVEs) in third-party libraries before they are merged. |
| Lockfiles | Always commit dependency lockfiles (`package-lock.json`, `requirements.txt`) and strictly install from them. | Ensures deterministic builds and prevents supply chain attacks through unexpected sub-dependency updates. |
| Private Registries | Proxy public registries through an internal artifact repository. | Protects against dependency confusion attacks and left-pad scenarios where public packages disappear. |
| Minimal Dependencies | Avoid adding third-party libraries for trivial logic that can easily be written natively. | Every dependency increases the attack surface and maintenance burden of the application. |
