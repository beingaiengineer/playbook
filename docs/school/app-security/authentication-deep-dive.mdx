...
{/* TODO: [BAE-ILLUSTRATION] Generate and embed illustration here. Prompt: A multi-layered vault door with biometric scanners and keypads. */}
...
|---|---|---|
| Secure Password Hashing | Use robust hashing algorithms like Argon2, bcrypt, or PBKDF2 with unique salts. Never store plaintext passwords. | Weak hashing or plaintext storage results in severe data breaches if the database is compromised. |
| Multi-Factor Authentication | Prompt for MFA on all sensitive actions and privileged accounts. | MFA provides a critical secondary layer of defense if primary credentials are stolen. |
| Token Expiration | Use short-lived Access Tokens and long-lived Refresh Tokens. Never use non-expiring tokens. | Reduces the window of opportunity for an attacker using a stolen token. |
| Secure Transport | All authentication requests must strictly be performed over TLS/HTTPS. | Prevents MITM (Man-in-the-Middle) attacks from intercepting plaintext credentials over the network. |
