...
|---|---|---|
| Concurrency Control | When fanning out API calls, use semaphores (e.g., `asyncio.Semaphore`) to limit concurrent requests. Do not fire 50+ LLM requests simultaneously. | Unbounded concurrency immediately triggers 429 Too Many Requests errors from LLM providers, crashing the entire map-reduce operation. |
| State Reduction | In Fan-In operations, only mutate state variables that are annotated with a Reducer (e.g., `Annotated[list, operator.add]`). Never overwrite global static state from a parallel worker. | Modifying static state keys concurrently causes race conditions, silent data loss, and unpredictable merge conflicts. |
| Graceful Degradation | Use `return_exceptions=True` in `asyncio.gather()` or wrap parallel node execution in `try/except` blocks. | A single failure (like a 404 URL in one parallel task) should not crash the other successful tasks during the synthesis phase. |
