...
Bro, imagine you're debugging a customer support bot that suddenly decided it had the authority to issue full refunds. The original team just used a generic 'chat agent' template without understanding the taxonomy of agent types. They gave a simple conversational agent the tools and permissions of an execution-focused autonomous agent. By redefining the system's architecture and clearly separating the 'router agent' from the 'execution agent', you restored order and prevented further financial loss. Knowing what type of agent to build is step one.
...
|---|---|---|
| Principle of Least Privilege | Only grant an agent the tools strictly necessary for its specific type and function. | Providing generic tools to all agents increases the risk of unintended actions and security breaches. |
| Explicit Typing | Clearly define and document the type (e.g., Router, Executor, Evaluator) of each agent in multi-agent systems. | Ambiguity in agent roles leads to overlapping responsibilities and unpredictable execution flows. |
| Role Boundaries | Prevent conversational agents from executing state-changing tools without explicit approval from an execution agent or human. | Mixing conversation and execution without boundaries is a common source of critical agent failures. |
