...
Bro, imagine you are a senior platform engineer building an ADK agent that needs to run complex analytics on BigQuery. Instead of writing custom Python wrapper functions, you use Google's highly optimized Native Vertex AI Tools to provide secure, direct access to enterprise datastores.

## 3. Real-World Analogy

![Real-World Analogy](../assets/google-adk/google-adk-tools-and-functions.png)

Imagine a master mechanic (the LLM). If he only has a wrench, he can only do basic repairs. If you give him the keys to the automated diagnostics machine and the hydraulic lift (Tools), he can overhaul the entire engine. In Google ADK, providing tools turns the LLM from a simple chat interface into a system that can read databases and trigger cloud functions.
...
|---|---|---|
| Tool Selection | Always prioritize Native GCP Tools (e.g., Vertex Search, BigQuery) over custom Python wrappers when available. | Native tools are optimized for latency, handle pagination automatically, and enforce GCP security perimeters. |
| Error Propagation | Custom tools must catch internal exceptions and return a clean, descriptive string back to the agent instead of raising raw Python errors. | Prevents the entire ADK orchestration loop from crashing and allows the LLM to understand and self-correct the error. |
| Payload Limits | Enforce strict payload size limits or summarization within custom tools before returning data to the agent. | Prevents massive JSON responses from overwhelming the Gemini context window and causing OOM or API rejections. |
