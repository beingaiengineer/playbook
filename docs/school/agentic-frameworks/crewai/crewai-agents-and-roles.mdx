...
Bro, imagine you are a senior engineer scaling a multi-agent system, but your agents keep hallucinating instructions because their roles and personas are poorly defined, leading to overlapping responsibilities and system failure.
...
In a production system, you don't hardcode API keys or rely on defaults. You also need to configure agents to use specific, cheaper LLMs for easy tasks, and expensive LLMs for hard tasks.
...
What happens if an Agent's `goal` directly contradicts the instructions given in its `Task`?
